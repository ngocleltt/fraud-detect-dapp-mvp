                ...
                {processedUsers.length > 0 ? (
                  processedUsers.map((user) => {
                    const isSelected =
                      ...
                      selectedUser?.target_address === user.target_address;

                    return (
                      <tr
                        key={`${user.user_id}-${user.target_address}`}
                        onClick={() => setSelectedUser(user)}
                        className={`group cursor-pointer text-sm text-slate-600 transition-colors duration-150 hover:bg-slate-200/70 ${
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-mono">
                          {formatNumber(user.features["avg val received"], 2, 3)}
                        </td>
